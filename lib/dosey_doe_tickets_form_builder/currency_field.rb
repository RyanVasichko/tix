module DoseyDoeTicketsFormBuilder
  module CurrencyField
    def currency_field(method, options = {})
      ...
      add_class_to_options(options, "form-control-text pl-7 pr-12")
      ...
      currency_code_span = @template.content_tag(:span, "USD", class: "text-gray-500 sm:text-sm", id: "price-currency")
      currency_code_div = @template.content_tag(:div, currency_code_span, class: "pointer-events-none absolute inset-y-0 right-0 flex items-center pr-3")
      ...
      currency_field_div = @template.content_tag(:div, class: "relative mt-2 rounded-md shadow-sm") do
        @template.safe_join([currency_symbol_div, number_field, currency_code_div])
      end
