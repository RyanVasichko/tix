module DoseyDoeTicketsFormBuilder
  module Combobox
    ...
    def collection_combobox(method, collection, value_method, text_method, options = {})
      choices = collection.pluck(value_method, text_method)

      disabled = options[:disabled] || false

      selected_value = choices.find { |choice| choice[0] == object.send(method) }
      input_value = selected_value ? selected_value[1] : ""

      @template.content_tag(:div, data: { controller: ("combobox" unless disabled), action: "click@document->combobox#hideOptionsOnOutsideClick" }) do
        combobox = @template.content_tag(:div, class: "relative mt-2") do
          hidden_field = hidden_field(method, data: { combobox_target: "hiddenField", action: "combobox--option:selected@window->combobox#optionSelected" })
          text_field_options = {
            name: nil,
            disabled: disabled,
          ...
          add_error_fields_to_options(text_field_options, method)
          text_field = @template.text_field_tag(combobox_text_field_id(method), input_value, text_field_options)
        ...
        @template.safe_join([combobox, error_message_for(method)])
      end
    end
