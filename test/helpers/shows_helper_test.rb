...
class ShowsHelperTest < ActionView::TestCase
  ...
  test 'preload_venue_layout_images generates unique preload links for each image' do
    ...
    2.times { FactoryBot.create(:show, venue_layout_blob: venue_layout_blob) }
    FactoryBot.create(:show, venue_layout_blob: lcd_soundsystem_blob)
    FactoryBot.create(:show, :past, venue_layout_blob: radiohead_blob)
